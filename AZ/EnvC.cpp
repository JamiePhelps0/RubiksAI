...
void generateDataset(int numSearches, int fileNum, int fidx) {
    filesystem::create_directory("/home/jamie/IdeaProjects/RubiksNew/AlphaZero/DataFiles/Dataset" + to_string(fidx));
    ofstream dataout;
    ...
    RubiksCube cube;
    cube.Scramble(3);
    cppflow::model net("/home/jamie/IdeaProjects/RubiksNew/DQN/savedmodel");
    ...
    for (int i = 0; i < numSearches; i++) {
        current++;
        Node* search = mcts(cube, 300, net);
        for (int j = 0; j < 12; j++) {
            dataout << (*search).q_values[j] << ';';
        }
        if (rand() % 10 == 0) {
            action = rand() % 12;
        }
...
int main() {
    ...
    int data_per_thread = 512;

    while(1) {

    ifstream ds_idx;
    ds_idx.open("/home/jamie/IdeaProjects/RubiksNew/AlphaZero/DataFiles/idx.txt");
    string st_idx;
    getline(ds_idx, st_idx);
    cout << endl << st_idx << endl;
    ds_idx.close();
    int idx = stoi(st_idx);

    thread threads[16] = {thread(generateDataset, data_per_thread, 0, idx), thread(generateDataset, data_per_thread, 1, idx), thread(generateDataset, data_per_thread, 2, idx), thread(generateDataset, data_per_thread, 3, idx),
                          thread(generateDataset, data_per_thread, 4, idx), thread(generateDataset, data_per_thread, 5, idx), thread(generateDataset, data_per_thread, 6, idx), thread(generateDataset, data_per_thread, 7, idx),
                          thread(generateDataset, data_per_thread, 8, idx), thread(generateDataset, data_per_thread, 9, idx), thread(generateDataset, data_per_thread, 10, idx), thread(generateDataset, data_per_thread, 11, idx),
                          thread(generateDataset, data_per_thread, 12, idx), thread(generateDataset, data_per_thread, 13, idx), thread(generateDataset, data_per_thread, 14, idx), thread(generateDataset, data_per_thread, 15, idx)};
    for (int i = 0; i < 16; i++) {
        threads[i].join();
    }
    ofstream file;

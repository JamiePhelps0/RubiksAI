...
class PrioritizedReplayBuffer:
    def __init__(self, buffer_size, eps=1e-2, alpha=0.6, beta=0.4):
        ...
        self.state = np.empty([buffer_size, 3, 3, 3], dtype=np.float32)
        self.action = np.empty(buffer_size, dtype=np.float32)
        self.reward = np.empty(buffer_size, dtype=np.float32)
        self.next_state = np.empty([buffer_size, 3, 3, 3], dtype=np.float32)
        self.done = np.empty(buffer_size, dtype=bool)
    ...
    def sample(self, batch_size):
        ...
        # To sample a mini-batch of size k, the range [0, p_total] is divided equally into k ranges.
